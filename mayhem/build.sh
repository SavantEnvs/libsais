...
# This UPSTREAM revision predates the PREFIX "" target property, so CMake's default
# naming ("lib" + target name) applies: liblibsais.a, not libsais.a.
LIBSAIS_A=$(find build -maxdepth 1 -name '*libsais.a')

# 2) Harness: libFuzzer binary + standalone run-once reproducer
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $LIB_FUZZING_ENGINE \
    "$SRC/mayhem/sais-fuzz.c" -I"$SRC/include" "$LIBSAIS_A" \
    -o /mayhem/sais-fuzz
...
LIBSAIS_TESTS_A=$(find build-tests -maxdepth 1 -name '*libsais.a')
$CC -O2 $COVERAGE_FLAGS "$SRC/mayhem/libsais_selftest.c" -I"$SRC/include" \
    "$LIBSAIS_TESTS_A" -o /mayhem/libsais_selftest
